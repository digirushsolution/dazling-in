...
if ( vamtam_theme_supports( 'woocommerce-product-images--sale-flash-section' ) ) {
	function add_vamtam_sale_flash_section( $controls_manager, $widget ) {
		...
		if ( vamtam_theme_supports( 'woocommerce-product-images--onsale-badge-theme-mask' ) ) {
			// Use Theme Mask.
			$widget->add_control(
...
if ( vamtam_theme_supports( 'woocommerce-product-images--onsale-badge-theme-mask' ) ) {
	...
	// Product Images, render_content.
	function product_images_render_content( $content, $widget ) {
		$widget_name = $widget->get_name();
		if ( $widget->get_name() === 'global' ) {
			$widget_name = $widget->get_original_element_instance()->get_name();
		}

		if ( $widget_name === 'woocommerce-product-images' ) {
			$settings               = $widget->get_settings();
			$should_inject_svg_path = ( vamtam_theme_supports( 'woocommerce-product-images--onsale-badge-theme-mask' ) && ! empty( $settings['vamtam_onsale_badge_use_theme_mask'] ) ) || ( vamtam_theme_supports( 'product-images--new-badge-section' ) && ! empty( $settings['vamtam_new_badge_use_theme_mask'] ) );
	...
	add_filter( 'elementor/widget/render_content', __NAMESPACE__ . '\product_images_render_content', 10, 2 );
}
