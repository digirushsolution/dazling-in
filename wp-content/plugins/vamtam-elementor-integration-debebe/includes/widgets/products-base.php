...
if ( vamtam_theme_supports( 'products-base--new-badge-section' ) ) {
	function add_new_badge_section( $controls_manager, $widget ) {
		...
		// Use Theme Mask.
		$widget->add_control(
			'vamtam_new_badge_use_theme_mask',
			[
				'label' => __( 'Use Theme Mask', 'vamtam-elementor-integration' ),
				'type' => $controls_manager::SWITCHER,
				'prefix_class' => 'vamtam-has-',
				'return_value' => 'new-badge-theme-mask',
				'default' => 'new-badge-theme-mask',
				'condition' => [
					'vamtam_show_new_badge!' => '',
				],
			]
...
if ( vamtam_theme_supports( [ 'products-base--onsale-badge-theme-mask', 'products-base--new-badge-section' ] ) ) {
	...
	function add_onsale_badge_use_theme_mask_control( $controls_manager, $widget ) {
		// Use Theme Mask.
		$widget->add_control(
			'vamtam_onsale_badge_use_theme_mask',
			[
				'label' => __( 'Use Theme Mask', 'vamtam-elementor-integration' ),
				'type' => $controls_manager::SWITCHER,
				'prefix_class' => 'vamtam-has-',
				'return_value' => 'onsale-badge-theme-mask',
				'default' => 'onsale-badge-theme-mask',
				'condition' => [
					'show_onsale_flash!' => '',
				],
			]
	...
	function sale_flash_style_before_section_end( $widget, $args ) {
		...
		if ( vamtam_theme_supports( 'products-base--onsale-badge-theme-mask' ) ) {
			add_onsale_badge_use_theme_mask_control( $controls_manager, $widget );
		}
	}
	...
	function products_base_render_content( $content, $widget ) {
		...
		if ( in_array( $widget_name, $products_widgets ) ) {
			...
			if ( \Vamtam_Elementor_Utils::is_widget_mod_active( $widget_name ) ) {
				...
				$should_inject_svg_path = ( vamtam_theme_supports( 'products-base--onsale-badge-theme-mask' ) && ! empty( $settings['vamtam_onsale_badge_use_theme_mask'] ) ) || ( vamtam_theme_supports( 'products-base--new-badge-section' ) && ! empty( $settings['vamtam_new_badge_use_theme_mask'] ) );
				if ( $should_inject_svg_path ) {
					if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
						// Editor.
						// Inject badge clip path svg (inline).
						$badge_clip_path = '<svg class="vamtam-svg-cp"> <clipPath id="vamtam-badge-cp" clipPathUnits="objectBoundingBox"><path d="M0.552,0.013 L0.552,0.013 C0.604,-0.014,0.667,0.003,0.699,0.052 L0.699,0.052 C0.719,0.083,0.752,0.102,0.789,0.104 L0.789,0.104 C0.847,0.107,0.893,0.153,0.896,0.211 L0.896,0.211 C0.898,0.248,0.917,0.281,0.948,0.301 L0.948,0.301 C0.997,0.333,1,0.396,0.987,0.448 L0.987,0.448 C0.971,0.481,0.971,0.519,0.987,0.552 L0.987,0.552 C1,0.604,0.997,0.667,0.948,0.699 L0.948,0.699 C0.917,0.719,0.898,0.752,0.896,0.789 L0.896,0.789 C0.893,0.847,0.847,0.893,0.789,0.896 L0.789,0.896 C0.752,0.898,0.719,0.917,0.699,0.948 L0.699,0.948 C0.667,0.997,0.604,1,0.552,0.987 L0.552,0.987 C0.519,0.971,0.481,0.971,0.448,0.987 L0.448,0.987 C0.396,1,0.333,0.997,0.301,0.948 L0.301,0.948 C0.281,0.917,0.248,0.898,0.211,0.896 L0.211,0.896 C0.153,0.893,0.107,0.847,0.104,0.789 L0.104,0.789 C0.102,0.752,0.083,0.719,0.052,0.699 L0.052,0.699 C0.003,0.667,-0.014,0.604,0.013,0.552 L0.013,0.552 C0.029,0.519,0.029,0.481,0.013,0.448 L0.013,0.448 C-0.014,0.396,0.003,0.333,0.052,0.301 L0.052,0.301 C0.083,0.281,0.102,0.248,0.104,0.211 L0.104,0.211 C0.107,0.153,0.153,0.107,0.211,0.104 L0.211,0.104 C0.248,0.102,0.281,0.083,0.301,0.052 L0.301,0.052 C0.333,0.003,0.396,-0.014,0.448,0.013 L0.448,0.013 C0.481,0.029,0.519,0.029,0.552,0.013"></path></clipPath></svg>';
						$content = $badge_clip_path . $content;
					} else {
						// Frontend.
						// Inject badge clip path svg (once per-page).
						if ( ! has_action( 'wp_footer', __NAMESPACE__ . '\vamtam_inject_products_badge_svg_cp' ) ) {
							add_action( 'wp_footer', __NAMESPACE__ . '\vamtam_inject_products_badge_svg_cp' );
						}
					}
				}
			}
	...
	add_filter( 'elementor/widget/render_content', __NAMESPACE__ . '\products_base_render_content', 10, 2 );
}
