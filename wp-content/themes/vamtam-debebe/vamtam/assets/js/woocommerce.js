( function( $, v, undefined ) {
	...
	$( function() {
		...
		$( document.body ).on( menuCartEvents, function() {
			function handleSubscriptionItem() {
				const $subItems = $elementorCart.find( '.product-price .quantity .subscription-details' );

				$.each( $subItems, function (i, el) {
					const $qtyEl = $( el ).parent( '.quantity' );
					$qtyEl.addClass( 'vamtam-has-sub-details' );
					$qtyEl.append( '<div class="vamtam-price-details"></div>' );
					$qtyEl.find( '> .amount, > .subscription-details' ).appendTo( $qtyEl.find( '.vamtam-price-details' ) );
				} );
			}
